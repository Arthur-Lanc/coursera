...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (int i = 0; i < word.length(); i++) {
			...
			if(i < word.length()-1) {
				if (vowels.indexOf(chLower) >= 0 && !IsPreCharVowel) {
					sylNum += 1;
					IsPreCharVowel = true;
				}else if(vowels.indexOf(chLower) >= 0){
					IsPreCharVowel = true;
				}else {
					IsPreCharVowel = false;
				}
			}else {
				if(vowels.indexOf(chLower) >= 0 && !IsPreCharVowel && chLower != 'e') {
					sylNum += 1;
					IsPreCharVowel = true;
				}else if (sylNum == 0 && chLower == 'e'){
					sylNum += 1;
					IsPreCharVowel = true;
				}else if(vowels.indexOf(chLower) >= 0){
					IsPreCharVowel = true;
