...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)(
      (acc, page) => if(page.mentionsLanguage(lang)) acc + 1 else acc,
      (acc1, acc2) => acc1 + acc2
    )
...
//    langs.map(lang => (lang, occurrencesOfLang(lang, rdd))).sortBy(_._2).reverse
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(article => langs.filter(lang => article.mentionsLanguage(lang))
...
//    result.collect.toList.sortWith(_._2 > _._2)
    ...
    index.mapValues(v => v.size).sortBy(_._2,false).collect.toList
  }
...
//    )
//    .reduceByKey(_ + _)
//    .sortBy(_._2,false).collect.toList

    rdd.flatMap(page => langs
      .filter(lang => page.mentionsLanguage(lang))
      .map(lang => (lang, 1)))
      .reduceByKey(_ + _)
