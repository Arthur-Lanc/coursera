...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toList.groupBy(_.toLower).map(x => x match {case (a,b) => (a, b.length)}).toList.sortWith((x,y) => x._1 < y._1)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def helper(o: Occurrences): List[Sentence] = {
      if (o.isEmpty) {
//        List(Nil)
        List[Sentence](Nil)
      } else {
        ...
        for (i <- combs if dictionaryByOccurrences.keySet(i);
             ...
             s <- helper(subtract(o, i))) yield {j :: s}
      }
    }
...
//    println("Hello, world!")
//    val sentence = List("i","love")
//    println(sentenceAnagrams(sentence))
//    println(sentenceAnagrams(List("i")))
